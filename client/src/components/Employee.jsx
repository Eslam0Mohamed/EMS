...
const Employee = () => {
  ...
  return (
    <div className='animate-fade-in'>
      ...
      {showCreateModal && <div onClick={() => { setShowCreateModal(false) }} className='fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start justify-center overflow-auto'>
      ...
      {edit && <div onClick={() => { setEdit(null) }} className='fixed bg-black/40 backdrop-blur-sm inset-0 z-50 flex items-start justify-center overflow-auto'>
        ...
        <div onClick={(e) => { e.stopPropagation() }} className='relative bg-white rounded-2xl shadow-2xl my-8 animate-fade-in max-w-3xl w-full'>
          ...
          <div className='p-5'>
            <EmployeeForm initialData={edit} onCancel={() => { setEdit(null) }} onSuccess={() => {
              setEdit(null); fetchEmployees();
            }} />
          </div>
        </div>
      </div>}
